...
function makeid() {
  ...
  function pegaFig (palavra){
      ...
      if (['1AEVE','2AEVE','3AEVE','4AEVE'].includes(palavra)) {return "/EVE.svg"} else
...
{bloco.map(
    (bl)=>{
        return <Container key={makeid()}>{(bl.bloco =="D"&&bl.andar=="2"?
        bl.disposicao.map((sl)=>{
          return <Card key={makeid()}>
            ...
            <div><p>Sala: D2{sl.sala}</p>  <p>Turma:{(turno!="tarde"?sl.turma.manhã:sl.turma.tarde)}</p> <p>Aula1:{(turno!="tarde"?sl.discM12.disciplina:sl.discT12.disciplina)}</p> <p>Aula 2:{(turno!="tarde"?sl.discM34.disciplina:sl.discT34.disciplina)}</p></div>
          </Card>
        }
        )
        :null)}</Container>
