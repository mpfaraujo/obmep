...
{bloco.map(
    (bl)=>{
        return <Container key={makeid()}>{(bl.bloco =="PAVILHAO"&&bl.andar=="1"?
        bl.disposicao.map((sl)=>{
          return <Card key={makeid()}>
            ...
            <div><p>Sala: {sl.sala}</p>  <p>Turma:{(turno!="tarde"?sl.turma.manhã:sl.turma.tarde)}</p> <p>Aula1:{(turno!="tarde"?sl.discM12.disciplina:sl.discT12.disciplina)}</p> <p>Aula 2:{(turno!="tarde"?sl.discM34.disciplina:sl.discT34.disciplina)}</p></div>
          </Card>
        }
        )
        :null)}</Container>
